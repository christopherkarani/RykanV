...
- **FS scope (Landlock):** workspace child RW with workspace-root RO — create/write at the workspace root is denied; Seatbelt (macOS) allows full workspace subpath RW including create-at-root. Default control roots are `{workspace}/.orca` and `{workspace}/.git` (RO under expand; siblings remain RW).
- **Narrow host-agent config RW:** empty backpack keeps bare `$HOME` denied. Known launch hosts get **existing** config trees only (same table as macOS). Missing roots are not invented.
