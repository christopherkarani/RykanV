...
2. Copy the ryk plugin file into the project-local plugins directory:
   ...
   cp /path/to/ryk-repo/integrations/opencode-plugin/orca.ts .opencode/plugins/ryk.ts
   ```

   Or create a symlink:

   ```bash
   ln -s /path/to/ryk-repo/integrations/opencode-plugin/orca.ts .opencode/plugins/ryk.ts
   ```
