...
test("storeSecretToEnvFile writes under .ryk/dev-secrets.env", () => {
	const cwd = mkdtempSync(resolve(tmpdir(), "ryk-pi-secret-"));
	try {
		storeSecretToEnvFile(cwd, "OPENAI_API_KEY", SYNTH_OPENAI);
		const path = resolve(cwd, ".ryk/dev-secrets.env");
...
test("storeSecretToEnvFile refuses symlinked credential directories and files", () => {
	...
	try {
		symlinkSync(outside, resolve(cwd, ".ryk"), "dir");
		assert.throws(
			() => storeSecretToEnvFile(cwd, "OPENAI_API_KEY", SYNTH_OPENAI),
...
test("storeSecretToEnvFile fails closed if the credential directory is swapped before rename", () => {
	...
	const moved = resolve(cwd, ".ryk-moved");
	try {
		mkdirSync(resolve(cwd, ".ryk"));
		assert.throws(
			() =>
				_storeSecretToEnvFileWithHooksForTest(
					...
					{
						beforeRename() {
							renameSync(resolve(cwd, ".ryk"), moved);
							symlinkSync(outside, resolve(cwd, ".ryk"), "dir");
						},
					},
				),
...
test("isInteractiveCaptureSession and disable flag", () => {
	...
	assert.equal(
		isSecretCaptureDisabled({ RYK_PI_SECRET_CAPTURE: "false" }),
		true,
	);
...
test("installOrcaExtension wires input capture and keeps bash evaluate", async () => {
	...
	const wireCwd = mkdtempSync(resolve(tmpdir(), "ryk-pi-wire-"));
	try {
		...
		assert.ok(
			existsSync(resolve(wireCwd, ".ryk/dev-secrets.env")),
...
test("bash tool_call path still present after secret capture wiring", async () => {
	// Reuse the same FakeChild pattern lightly: allow when ryk returns allow.
