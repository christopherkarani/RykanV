...
// Synthetic host_rw (not basename spoof). Exit: 0=ok, 2=apply fail, 3=cross link
// succeeded (leak), 4=workspace-only link failed, 5=workspace write failed.
test "real FS: F-03 host-config hardlink into workspace denied; workspace-only link ok" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, sbpl_z, "(allow file-link (subpath \"") != null);
