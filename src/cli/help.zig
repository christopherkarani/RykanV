    ...
    [_]CommandInfo{
        .{
            ...
            .details = &.{
                ...
                "Agent-primary defaults for host aliases (ryk pi, ryk claude, …): network mode allowlist + proxy backend + OS route-force (fail closed if mediation cannot start). Non-alias `ryk run -- <cmd>` still defaults network mode to ask without forcing proxy. Secretless stays off unless --secretless. Opt-outs: --network open|allowlist|observe|off|ask, --no-network.",
