...
VERSION="${RYK_VERSION:-$(tr -d '[:space:]' < "${SCRIPT_DIR}/../VERSION")}"
DIST_DIR="${RYK_DIST_DIR:-dist}"
OUT_DIR="${RYK_PACKAGE_MANIFEST_DIR:-${DIST_DIR}/package-manifests}"
...
windows_amd64="$(checksum_for "ryk-v${VERSION}-windows-amd64.zip")"
...
sed \
  ...
  -e "s|https://github.com/christopherkarani/ryk/releases/download/v[^\"]*|https://github.com/christopherkarani/ryk/releases/download/v${VERSION}|" \
...
if [ -f packaging/scoop/ryk.json ]; then
  sed \
    ...
    -e "s/PLACEHOLDER_WINDOWS_AMD64_SHA256/${windows_amd64}/g" \
    packaging/scoop/ryk.json > "$OUT_DIR/scoop/ryk.json"
fi
if [ -f packaging/winget/ryk.yaml ]; then
  sed \
    ...
    -e "s|v[0-9][0-9.]*|v${VERSION}|g" \
    -e "s/PLACEHOLDER_WINDOWS_AMD64_SHA256/${windows_amd64}/g" \
    packaging/winget/ryk.yaml > "$OUT_DIR/winget/ryk.yaml"
fi

for rendered in \
  "$OUT_DIR/homebrew/Formula/ryk.rb" \
  "$OUT_DIR/npm/package.json" \
  "$OUT_DIR/scoop/ryk.json" \
  "$OUT_DIR/winget/ryk.yaml"
do
  [ -f "$rendered" ] || continue
