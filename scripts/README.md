...
- `update-homebrew-formula.sh`: updates `packaging/homebrew/Formula/ryk.rb` from `dist/checksums.txt`.
...
- `build-release.sh`: builds cross-platform release archives into `dist/`; production builds require `RYK_POSTHOG_PROJECT_TOKEN`, while local dry-runs can set `RYK_TELEMETRY_BUILD_DISABLED=1`.
